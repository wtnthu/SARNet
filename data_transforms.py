...
class Compose(object):
    ...
    def __call__(self, f, tt , gt, id):
        for t in self.transforms:
            f, tt, gt = t(f, tt, gt, id)
        return f, tt, gt
...
class To_nor(object):
    ...
    def __call__(self, img_f, img_t, img_gt):
...
class RandomCrop(object):
    ...
    def __call__(self, img_f, img_t, img_gt, id):
        '''
...
class ToTensor(object):
    ...
    def __call__(self, img_f, img_t, img_gt, id):
        ...
        '''
        img_f, img_t, img_gt = img_f.astype(np.float32), img_t.astype(np.float32), img_gt.astype(np.float32)
